...
<?php endif; ?>
            ...
            $(".client").autocomplete({
                        ...
                        success: function (data) {
                            response(jQuery.map(data, function (item) {
                                return {
									...
									sms_versaire : item.sms_versaire
                                }
                            }));
                        }
                    });
                ...
                select: function (event, ui) {
					...
					if(ui.item.sms_versaire=="0" || ui.item.sms_versaire==0)
					{
						$("input[name='sms']").parent(".dhx_cal_wide_checkbox").parent(".dhx_wrap_section").hide();
					}
					else{
						$("input[name='sms']").parent(".dhx_cal_wide_checkbox").parent(".dhx_wrap_section").show();
					}
                    return false;
                }
        ...
        scheduler.attachEvent("onBeforeDrag", function (ev, mode) {
            if (mode === 'move') {
				...
				console.log("debut before drag");
				
				console.log(dragged_event);
				console.log("fin before drag");
            }
			console.log("ggg");
            return true;
        });
		...
		scheduler.attachEvent("onDragEnd", function(event,mod){
			...
			*/
			...
			console.log(event);*/
			if(mod=="move"){
				console.log("debut end drag");
				
				console.log(event);
				console.log("fin end drag");
				$.ajax({
					url: "<?php echo site_url('user/ajax/save_event') ?>",
					type: "POST",
					dataType: "json",
        ...
        scheduler.renderEvent = function (container, ev, width, height, header_content, body_content) {
            ...
            if (ev.sms === "1") { // rdv sms notification allowed
                /*$.ajax({
				 ...
				 html += '<img class="sms_sent" src="<?php echo site_url('assets/backend/design/message.png') ?>" title="SMS de notification de RDV envoyé" alt="SMS de notification de RDV envoyé"/>';
            }
